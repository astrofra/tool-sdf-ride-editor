...
const char *mask_name(ModifierMask mask)
{
  switch (mask)
  {
...
bool save_scene_file(
  ...
  std::string *error_message)
{
  if (scene_file.scene.name.empty())
  {
    if (error_message != nullptr)
    {
  ...
  if (!validate_modifier_targets(scene_file.scene, output_path, error_message))
  {
    return false;
  }
  ...
  if (!output_path.parent_path().empty())
  {
    ...
    if (directory_error)
    {
      if (error_message != nullptr)
      {
        *error_message = "failed to create scene file parent directory: " + directory_error.message();
      }
      return false;
    }
  }
  ...
  if (!stream.is_open())
  {
    if (error_message != nullptr)
    {
      *error_message = "failed to open scene file for writing: " + output_path.string();
    }
    return false;
  }

  stream << "# Minimal line-based scene format for the first native slice.\n";
  stream << "# This file is generated by save_scene_file and remains fully editable.\n";
  stream << "scene " << scene_file.scene.name << '\n';
  stream << "bounds "
         << format_float(scene_file.build_settings.bounds.min.x) << ' '
         << format_float(scene_file.build_settings.bounds.min.y) << ' '
         << format_float(scene_file.build_settings.bounds.min.z) << ' '
         << format_float(scene_file.build_settings.bounds.max.x) << ' '
         << format_float(scene_file.build_settings.bounds.max.y) << ' '
         << format_float(scene_file.build_settings.bounds.max.z) << '\n';
  stream << "cell_size " << format_float(scene_file.build_settings.cell_size) << '\n';
  stream << "meshing_mode " << meshing_mode_name(scene_file.build_settings.meshing_mode) << '\n';
  stream << '\n';

  for (const SdfBox &box : scene_file.scene.boxes)
  {
    stream << "box "
           << csg_op_name(box.op) << ' '
           << box.name << ' '
           << format_float(box.transform.translation.x) << ' '
           << format_float(box.transform.translation.y) << ' '
           << format_float(box.transform.translation.z) << ' '
           << format_float(box.half_size.x) << ' '
           << format_float(box.half_size.y) << ' '
           << format_float(box.half_size.z);
    if (box.material_id != 0)
    {
      stream << ' ' << box.material_id;
    }
    stream << '\n';
  }

  if (!scene_file.scene.noise_modifiers.empty())
  {
    stream << '\n';
    for (const NoiseDisplaceMaskedModifier &modifier : scene_file.scene.noise_modifiers)
    {
      stream << "noise_displace_masked "
             << modifier.name << ' '
             << modifier.target_box_name << ' '
             << format_float(modifier.amplitude) << ' '
             << format_float(modifier.frequency) << ' '
             << modifier.seed << ' '
             << modifier.octaves << ' '
             << mask_name(modifier.mask) << ' '
             << format_float(modifier.mask_width) << '\n';
    }
  }

  if (!scene_file.scene.box_cut_modifiers.empty())
  {
    stream << '\n';
    for (const BoxCutModifier &modifier : scene_file.scene.box_cut_modifiers)
    {
      stream << "box_cut "
             << modifier.name << ' '
             << modifier.target_box_name << ' '
             << format_float(modifier.translation.x) << ' '
             << format_float(modifier.translation.y) << ' '
             << format_float(modifier.translation.z) << ' '
             << format_float(modifier.half_size.x) << ' '
             << format_float(modifier.half_size.y) << ' '
             << format_float(modifier.half_size.z) << '\n';
    }
  }
  ...
  if (!stream.good())
  {
    if (error_message != nullptr)
    {
      *error_message = "failed while writing scene file: " + output_path.string();
    }
    return false;
  }
